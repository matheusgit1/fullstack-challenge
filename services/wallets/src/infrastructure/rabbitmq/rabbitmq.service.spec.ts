...
describe('RabbitmqService', () => {
  ...
  describe('sucess scenarios', () => {
    it('should process transaction', async () => {
      mockTransactionRepository.findByExternalIdAndSource.mockResolvedValueOnce(null);
      await rabbitmqService.processReserve({ externalId: 'externalId' } as any, 'tracingId');
      expect(mockTransactionRepository.findByExternalIdAndSource).toHaveBeenCalledWith(
        'externalId',
        TransactionSource.BET_RESERVE,
      );
      expect(mockTransactionRepository.processTransaction).toHaveBeenCalledTimes(1);
    });
    ...
    it('should stop process transaction if troansactio already exist', async () => {
      ...
      expect(mockTransactionRepository.processTransaction).toHaveBeenCalledTimes(0);
    });

    it('should stop process cashin if transactio does not exist', async () => {
      mockTransactionRepository.findByExternalIdAndSource.mockResolvedValueOnce(null);

      await rabbitmqService.processCashin({ externalId: 'externalId' } as any, 'tracingId');
      expect(mockTransactionRepository.findByExternalIdAndSource).toHaveBeenCalledWith(
        'externalId',
        TransactionSource.BET_RESERVE,
      );
    ...
    it('should stop process cashout if transaction does not exist', async () => {
      ...
      await rabbitmqService.processCashout({ externalId: 'externalId' } as any, 'tracingId');
      expect(mockTransactionRepository.findByExternalIdAndSource).toHaveBeenCalledWith(
        'externalId',
        TransactionSource.BET_RESERVE,
      );
